...
export async function showVerifiedRewardedAd(id: string, name: string): Promise<VerifiedActionResult> {
    ...
    try {
        ...
        if (completed === true) {
            void recordAnalytics("rewarded_ad_complete", { ad_display_id: id });
            void recordAnalytics("rewarded_ad_watched", { ad_display_id: id });
        } else {
            // The decline branch: offered and watched alone cannot separate a
            // refusal from an ad that never filled.
            void recordAnalytics("rewarded_ad_dismissed", { ad_display_id: id });
        }
...
export async function purchaseVerifiedShopItem(itemId: string, idempotencyKey: string): Promise<VerifiedActionResult> {
    ...
    try {
        ...
        void recordAnalytics("iap_purchase_started", { item_id: itemId });
        ...
        const fulfilled = result?.success === true && result.order?.status === "fulfilled";
        void recordAnalytics(fulfilled ? "iap_purchase_complete" : "iap_purchase_failed", {
            item_id: itemId,
            order_status: result?.order?.status ?? "none",
        });
...
/**
 * Submits lifetime services completed to the default leaderboard.
 *
 * This game has no score — a service is won or lost, not scored. Services
 * completed is monotonic, directly comparable between players, and already the
 * progression the results screen shows, so it is what the boards rank. The
 * boards shipped configured and empty because nothing ever submitted.
 * Fire-and-forget; never fabricates acceptance.
 */
export async function submitServicesCompleted(services: number, durationSeconds: number): Promise<number | null> {
    const api = RundotGameAPI as unknown as Record<string, unknown>;
    if (typeof api.leaderboard !== "object" || api.leaderboard === null || services <= 0) return null;
    try {
        const result = await withTimeout(
            RundotGameAPI.leaderboard.submitScore({
                score: Math.max(0, Math.round(services)),
...
const LIKE_PROMPT_KEY = "whisk-and-fury:like-prompt:v1";
