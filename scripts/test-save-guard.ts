...
const realSave = JSON.stringify({
    ...
    settings: {},
    progress: { totalPlays: 777, servicesCompleted: 12 },
});
...
store.patch({ totalPlays: store.get().totalPlays + 5 });
...
await tick(2_300);
expect(store.get().totalPlays === 777, `retry should apply the real save (totalPlays ${store.get().totalPlays})`);
expect(store.get().servicesCompleted === 12, "retry should restore servicesCompleted");
store.patch({ totalPlays: 800 });
expect((await saveSystem.flush()) === true, "flush should succeed once the cloud read succeeded");
expect(JSON.parse(cloud.get(SAVE_KEY) ?? "{}").progress?.totalPlays === 800, "verified flush should reach the cloud");

// 3. A save from a newer build is never overwritten.
const newer = JSON.stringify({ version: SAVE_VERSION + 1, settings: {}, progress: { totalPlays: 5 } });
cloud.set(SAVE_KEY, newer);
const fresh = await import(`../src/systems/save.ts?newer`);
await fresh.saveSystem.load();
store.patch({ totalPlays: 1 });
